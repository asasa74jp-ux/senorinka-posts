...
## 毎朝9時JSTトレンドレポートの標準フォーマット（2026-09-13〜）

SocialData接続確認（2026-09-13）以降、毎朝の自動トレンドレポートは以下を標準とする（本人指示）。

- **平常比（ratio）で異常検知する**: 監視アカウントごとに非固定ツイートのviews中央値を平常値とし、
  直近24〜48時間の投稿がその何倍かを算出して急伸を判定する（fetch_today.pyのデータで計算可能）。
  「views・いいね・保存の絶対値」だけでなく「そのアカウントの平常時の何倍か」を必ず併記する。
- **各項目に投稿へのリンクを付ける**: `https://x.com/{screen_name}/status/{id_str}` の形式で、
  レポートの各箇条書きに元投稿のURLを添える（本人が実物を確認できるようにするため）。
- レポート本体（チャット表示）とbanks/format_bank.mdへのログ、両方にリンクを含める。
- この標準は本人が「やめて」「フォーマットを変えて」等、明示的に変更を指示するまで維持する。
